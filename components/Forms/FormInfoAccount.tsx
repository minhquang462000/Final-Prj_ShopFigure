...
export default function FormInfoAccount({ data }: IFormInfoAccountProps) {
  return (
    <div className='w-full flex gap-4 text-sm  bg-white  p-4 rounded-xl  flex-col'>
      <h2 className='text-2xl   mb-1'>Thông tin tài khoản</h2>
      <div className='flex justify-between items-center'>
        <label className='w-1/4' htmlFor="">Tên người dùng</label>
        <input className='w-3/4 outline-none bg-transparent border  px-3 py-1 rounded-full'
          defaultValue={data?.name}
          type="text" name="name" id="" placeholder="tên" />
      </div>
      <div className='flex justify-between items-center'>
      ...
      <div className='flex justify-between items-center'>
        ...
        <input defaultValue={data?.address} className='w-3/4 outline-none bg-transparent border  px-3 py-1 rounded-full' type="text" name="name" id="" placeholder="Nhập địa chỉ" />
      </div>
      <div className='flex justify-between items-center'>
