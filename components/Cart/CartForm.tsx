...
export default function CartForm(props: ICartFormProps) {
    ...
    const arrCount = cart.productQuantity.map((item:any) => {
    ...
    for (let i = 0; i < cart.product.length; i++) {
        ...
        for (let j = 0; j < arrCount.length; j++) {
            const element2 = arrCount[j];
            if (element.product_id == element2.product) {
              total = (element.price - (element.price * element.discount) / 100) * element2.count
            }
        }
        totalAll += total
    }
      ...
      <main className="w-[1280px] grid grid-cols-4 gap-4  mb-8 text-black mx-auto">
          ...
          <section className="col-span-3 flex flex-col gap-4 w-full">
              <div className="bg-white p-4 w-full" >
                  <nav className="flex   py-3 justify-between items-center">
                      ...
                      <p className="text-sm border-b-[1px] font-medium  border-black">{cart.product.length} Sản phẩm</p>
                  </nav>
                  <div className="flex flex-col gap-3">
                      {cart.product?.map((item: IProduct, index: number) => (
                          <CartItemCart key={index} data={item}     count={cart.productQuantity} />
                      ))}
          ...
          <div className="col-span-1 flex flex-col gap-4  bg-white p-4">
              ...
              <nav className="flex border-y-[1px] items-center py-3 justify-between font-medium text-xl">
                  <h2>Tổng tiền:</h2>
                  <p className="text-2xl text-[#d70018]">{addDotToNumber(String(totalAll))}đ</p>
              </nav>
