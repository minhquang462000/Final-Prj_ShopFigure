...
export default function ListCharacter (props:  ListCharacterProps) {
    const {characters , query, total , searchParams} = props
   ...
   useEffect(() => {
    ...
    const onSelectAll = (event: any) => {
      let checked = event.target.checked;
      if (checked) {
        const temp = characters.map((item: ICharacter) => item?.character_id);
        setIdsRemove(temp);
      } else {
        setIdsRemove([]);
      }
        ...
        {characters.map((character: ICharacter, key: number) => (
          ...
          >
            ...
            <input
              type="checkbox"
              checked={
                idsRemove.includes(character?.character_id) ? true : false
              }
